...
mod tests {
    ...
    #[test]
    fn test_insert_and_height() {
        let mut side = Side::<TestOrder>::new(true);

        side.insert_order(TestOrder::new("1", true, 100, 50));
        assert_eq!(side.height(), 1);

        // Same price - no new level
        side.insert_order(TestOrder::new("2", true, 100, 30));
        assert_eq!(side.height(), 1);

        // Different prices - new levels
        side.insert_order(TestOrder::new("3", true, 200, 30));
        side.insert_order(TestOrder::new("4", true, 150, 20));
    ...
    fn test_remove_order() {
        ...
        // Remove last order at level
        let mut side = Side::<TestOrder>::new(true);
        let node_ptr = side.insert_order(TestOrder::new("1", true, 100, 50));
        side.remove_order(node_ptr);
        assert_eq!(side.iter().count(), 0);
    }

    #[test]
    fn test_iterators() {
        // Bids: highest to lowest
        let mut bids = Side::<TestOrder>::new(true);
        bids.insert_order(TestOrder::new("1", true, 100, 50));
        bids.insert_order(TestOrder::new("2", true, 300, 30));
        bids.insert_order(TestOrder::new("3", true, 200, 20));
        let prices: Vec<u64> = bids.iter().map(|o| o.price()).collect();
        assert_eq!(prices, vec![300, 200, 100]);

        // Asks: lowest to highest
        let mut asks = Side::<TestOrder>::new(false);
        asks.insert_order(TestOrder::new("1", false, 100, 50));
        asks.insert_order(TestOrder::new("2", false, 300, 30));
        asks.insert_order(TestOrder::new("3", false, 200, 20));
        let prices: Vec<u64> = asks.iter().map(|o| o.price()).collect();
        assert_eq!(prices, vec![100, 200, 300]);

        // Mutable iterator
        for order in bids.iter_mut() {
            let _ = order.price();
        }
        assert_eq!(bids.height(), 3);
    }
}
